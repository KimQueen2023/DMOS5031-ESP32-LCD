...
/* Full-screen double buffers in PSRAM (172×320×2 = ~110 KB each) */
...
void LVGL_Init(void)
{
    ...
    /* Allocate half-screen double buffers in PSRAM (~55 KB each).
     * If PSRAM fails (e.g. cold-boot timing), fall back to internal SRAM.
     * If BOTH fail, log fatal error and abort — LVGL cannot render without buffers. */
    ...
    if (!buf1 || !buf2) {
        ESP_LOGW(TAG_LVGL, "PSRAM buffer failed, trying SRAM fallback");
        if (!buf1) buf1 = heap_caps_malloc(LVGL_BUF_SIZE * sizeof(lv_color_t), MALLOC_CAP_8BIT);
        if (!buf2) buf2 = heap_caps_malloc(LVGL_BUF_SIZE * sizeof(lv_color_t), MALLOC_CAP_8BIT);
    }
    if (!buf1 || !buf2) {
        ESP_LOGE(TAG_LVGL, "FATAL: cannot allocate LVGL draw buffers — aborting");
        abort();
    }
    ESP_LOGI(TAG_LVGL, "Draw buffers: %d bytes each, buf1=%p buf2=%p",
             (int)(LVGL_BUF_SIZE * sizeof(lv_color_t)), buf1, buf2);
